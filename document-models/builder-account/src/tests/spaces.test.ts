...
describe('Spaces Operations', () => {
    ...
    it('should handle removeSpace operation', () => {
        const input: RemoveSpaceInput = generateMock(
            z.RemoveSpaceInputSchema(),
        );

        const updatedDocument = reducer(
            document,
            creators.removeSpace(input),
        );
    ...
    it('should handle addPackageToSpace operation', () => {
        const input: AddPackageToSpaceInput = generateMock(
            z.AddPackageToSpaceInputSchema(),
        );

        const updatedDocument = reducer(
            document,
            creators.addPackageToSpace(input),
        );
    ...
    it('should handle removePackageFromSpace operation', () => {
        const input: RemovePackageFromSpaceInput = generateMock(
            z.RemovePackageFromSpaceInputSchema(),
        );

        const updatedDocument = reducer(
            document,
            creators.removePackageFromSpace(input),
        );
